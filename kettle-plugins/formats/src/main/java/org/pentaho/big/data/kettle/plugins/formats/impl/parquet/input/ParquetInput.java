...
public class ParquetInput extends BaseFileInputStep<ParquetInputMeta, ParquetInputData> {
  ...
  public static List<IParquetInputField> createSchemaFromMeta( ParquetInputMetaBase meta ) {
    List<IParquetInputField> fields = new ArrayList<>(); for ( ParquetInputField f : meta.getInputFields() ) {
      fields.add( f );
    } return fields;
  }

  @Override public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    meta = (ParquetInputMeta) smi; data = (ParquetInputData) sdi;
  ...
  void initSplits() throws Exception {
    ...
    } String[] resolvedInputFileNames = new String[meta.inputFiles.fileName.length]; int i = 0;
    ...
    if ( meta.isIgnoreEmptyFolder() && ( actualFileFields.isEmpty() ) ) {
      data.splits = new ArrayList<>(); logBasic( "No Parquet input files found." );
    } else {
      ...
      data.splits = data.input.getSplits(); logDebug( "Input split count: {0}", data.splits.size() );
    }
    data.currentSplit = 0;
  }
  ...
  void openReader( ParquetInputData data ) throws Exception {
    logDebug( "Open split {0}", data.currentSplit ); IPentahoInputSplit sp = data.splits.get( data.currentSplit );
    data.reader = data.input.createRecordReader( sp ); data.rowIterator = data.reader.iterator();
  }
