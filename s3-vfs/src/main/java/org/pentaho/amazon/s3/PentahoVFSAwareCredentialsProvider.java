...
import org.pentaho.di.connections.ConnectionDetailsListener;
...
public class PentahoVFSAwareCredentialsProvider implements AWSCredentialsProvider, ConnectionDetailsListener {
  ...
  public PentahoVFSAwareCredentialsProvider() {
    connectionManagerSupplier.get().addConnectionDetailsListener( this ); findDefaultCredentials();
  }

  private void findDefaultCredentials() {
    defaultConnection =
        getConnectionDetails().stream().filter(
  ...
  @Override public void connectionUpdated( ConnectionDetails connectionDetails ) {
    if ( defaultConnection != null ) {
      if ( defaultConnection.getName().equals( connectionDetails.getName() ) ) {
  ...
  @Override public void connectionAdded( ConnectionDetails connectionDetails ) {
